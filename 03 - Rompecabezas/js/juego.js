...
var instrucciones = ["Mueve las piezas con las flechas del teclado", "Ordena las piezas para obtener la imagen objetivo", "Diviertete!"];
...
function chequearSiGano() {
  for(var i = 0; i < grilla.length; i++) {
    for(var j = 0; j < grilla[i].length; j++) {
      var pieza = grilla[i][j];
      var posicion = (3*i)+j+1;
      if(pieza !== posicion) {
        return false;
      }
    }
...
function mostrarCartelGanador() {
  if(chequearSiGano()){
    var respuesta = confirm("¡FELICITACIONES!\n¿Queres jugar de nuevo?");
    if(respuesta){
      mezclarPiezas(30);
      capturarTeclas();
    }
