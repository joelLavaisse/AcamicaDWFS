...
/* En este tipo de enemigo, el valor de la velocidad indica la velocidad máxima
a la que se moverá el enemigo, aleatorizando su valor entre 0 y el valor ingresado. */
...
var ZombieCaminante = function(sprite, x, y, ancho, alto, velocidad, rangoMov) {
  ...
  /* Aplicamos Math.random() a la velocidad del objeto para aleatorizar la velocidad
  y así tener enemigos con distintos niveles de dificultad. */
  this.velocidad = velocidad * Math.random()
}
...
ZombieCaminante.prototype.mover = function() {
  ...
  /* En esta parte lo que hacemos es devolver al lado derecho de la pantalla si
  toca el limite izquierdo, posicionandolo de forma aleatoria en el eje y del cavas.*/
  if (this.x < this.rangoMov.desdeX - 2*this.ancho){
    this.x = this.rangoMov.hastaX + 2*this.ancho;
    this.y = Math.random()*this.rangoMov.hastaY;
  }
